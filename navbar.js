...
async function deleteFavorite(user, favId) {
    ...
    // After deleting, we need to re-render and then re-sequence the order.
    await updateFavList(user);
    const listContainer = document.getElementById('fav-list-container');
    if (listContainer) {
        await updateOrderInFirestore(user, listContainer);
        await updateFavList(user); // re-render again with correct order numbers if needed
    }
}
...
async function toggleFavorite(user) {
    ...
    if (favBtn.classList.contains('active')) {
        await deleteFavorite(user, safeId); // Use the delete function to handle re-ordering
    } else {
    ...
    if (user) {
        ...
        listContainer.addEventListener('dragend', async (e) => {
            if (draggedItem) {
                ...
                await updateOrderInFirestore(user, listContainer);
                draggedItem = null;
                await updateFavList(user); // Re-render to ensure consistency
            }
        });
