...
class LanAI:
    def __init__(self, model_path="lanai_memory.json"):
        ...
        self.training_data = [
            ...
            (["what", "is", "your", "name"], ["i", "am", "lanai", "version", "0.3.2"]),
        ...
        if not self.load_model():
            ...
            self.vocab = [
                ...
                "a", "the", "an", "this", "that", "my", "working", "system", "version", "0.3.2"
            ]
    ...
    def chat(self):
        print("="*60)
        print("         LanAI 0.3.2 (Memory Persistent & Dynamic GRU)         ")
        print("="*60)
        ...
        while True:
            try:
                ...
                # [更新 3] 按需訓練 (Dynamic Training Optimization)
                # 若學到新字，或是使用者給了較長的輸入，就進行局部微調
                if new_words_learned:
                    print("[動態微調] 吸收新知識中...", end="\r")
                    # 把新學的字假裝成需要回覆的內容放入記憶，這是一個簡化的「單樣本學習」
                    self.training_data.append((words, ["i", "learned", "about"] + words))
                    self.train_epochs(50, initial_lr=0.03)
                else:
                    # 一般對話僅做極輕量的微調(預防過熱，提升回應速度)
                    self.train_epochs(5, initial_lr=0.01)
                
                indices = [self.word_to_idx.get(w, 0) for w in words]
                
                # 使用 Temperature 取樣 (可根據對話情境調整，0.2 保守，0.8 創意)
                res_indices = self.generate_response(indices, temperature=0.5)
